...
try:
    income_range = [float(i) for i in input("Введите диапазон доходов через пробел: ").split(' ')]
...
csv_writer = csv.writer(new_file, dialect=csv.excel)
csv_writer.writerow(csv_reader_list[0])
csv_writer.writerows([i for i in csv_reader_list[1:] if income_range[0] <= float(i[2]) <= income_range[1]])
new_file.close()

new_file = open("inflation_sorted_countries.csv", mode="w+", newline='')
csv_writer = csv.writer(new_file, dialect=csv.excel)
csv_writer.writerow(csv_reader_list[0])
csv_writer.writerows(sorted([i for i in csv_reader_list[1:]], key=lambda x: float(x[3])))
new_file.close()
